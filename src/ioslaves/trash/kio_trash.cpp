...
void TrashProtocol::stat(const QUrl &url)
{
    ...
    if (path.isEmpty() || path == QLatin1String("/")) {
        ...
        qCWarning(KIO_TRASH) << "parsed" << url << "got" << trashId << fileId << relativePath;

        const QString filePath = impl.physicalPath(trashId, fileId, relativePath);
        qCWarning(KIO_TRASH) << "filePath:" << filePath;
        if (filePath.isEmpty()) {
            error(impl.lastErrorCode(), impl.lastErrorMessage());
            return;
        }
        ...
        qCWarning(KIO_TRASH) << "link dest:" << entry.stringValue(KIO::UDSEntry::UDS_LINK_DEST);
        qCWarning(KIO_TRASH) << "origin path:" << entry.stringValue(KIO::UDSEntry::UDS_EXTRA);
        statEntry(entry);
        finished();
    }
}
                                   ...
                                   const TrashedFileInfo &info)
{
    ...
    entry.fastInsert(KIO::UDSEntry::UDS_EXTRA + 2, physicalPath);
    qCWarning(KIO_TRASH) << "physicalPath:" << physicalPath;
    return true;
}
